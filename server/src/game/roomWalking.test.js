...
test('rejects non-finite, outside-room, wrong-room and disconnected updates',()=>{
  ...
  for(const invalid of [null,{...pose,x:NaN},{...pose,z:Infinity},{...pose,x:1000},{...pose,room:'Office'}])assert.equal(acceptRoomWalk(game,player.id,invalid),null);
...
test('room exit and game completion discard stale positions',()=>{
  ...
  player.position={room:'Office',cell:null};assert.deepEqual(walkingSnapshot(game),[]);
