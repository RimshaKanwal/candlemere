...
function fixture() {
 ...
 game.start();for(const player of game.players)player.position={room:'Music Room',cell:null};
...
test('friends share one drawer state, and snapshots preserve it without changing game rules',()=>{
 ...
 assert.equal(acceptInteraction(game,a.id,{room:'Music Room',kind:'drawer'},1000).open,true);
 assert.equal(acceptInteraction(game,b.id,{room:'Music Room',kind:'drawer'},1001).open,false);
...
test('interactions require being inside the room and reject invalid objects or spam',()=>{
 ...
 for(const input of [null,{room:'Kitchen',kind:'drawer'},{room:'Music Room',kind:'solution'},{room:'Music Room',kind:'door'}])assert.equal(acceptInteraction(game,a.id,input),null);
 assert.ok(acceptInteraction(game,a.id,{room:'Music Room',kind:'piano'},1000));
 assert.equal(acceptInteraction(game,a.id,{room:'Music Room',kind:'drawer'},1100),null);
 a.connected=false;assert.equal(acceptInteraction(game,a.id,{room:'Music Room',kind:'drawer'},5000),null);
});
test('piano has a shared cooldown and finished matches expose no active props',()=>{
 const {game,a,b}=fixture(), input={room:'Music Room',kind:'piano'};
...
test('transport broadcasts to the whole game, including the actor, and rejects seat spoofing',()=>{
 ...
 receive({code:game.code,playerId:'someone-else',room:'Music Room',kind:'drawer'});assert.equal(messages.length,0);
 receive({code:game.code,playerId:a.id,room:'Music Room',kind:'drawer'});assert.equal(messages.length,1);
